...
window.electronAPI.onUpdateDownloaded(() => {
    ...
    btnInstall.id = 'btn-install-now'; // ID unik agar tidak double
    ...
    // Pastikan tidak ada tombol double
    const existingBtn = document.getElementById('btn-install-now');
    if (!existingBtn) {
...
window.electronAPI.onUpdateNotAvailable(() => {
    showToast('Aplikasi sudah versi terbaru! (v1.0.0)');
});

window.electronAPI.onUpdateError((msg) => {
    showToast('Gagal cek update: ' + msg);
});
